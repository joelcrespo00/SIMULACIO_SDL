...
class ServerB2:
    ...
    def recullEntitat(self, time, entitat):
        ...
        event_serverR = Event(self.server, "NEW SERVICE R FROM B2", event_proces.time, entitat)
        self.tractarEsdeveniment(event_serverR)


    def enviarEsdevenimentServerR(self, event):
        self.server.recullEntitat(event.entity, 2)
    ...
    def tractarEsdeveniment(self, event):
        ...
        if event.type== 'FINISH PROCESS SERVERR':
            event_fi_servei = Event(self.queue, "FINISH PROCESS SERVERB2", event.time, event.entity)
